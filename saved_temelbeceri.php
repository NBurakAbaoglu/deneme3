...
try {
    ...
    if (!isset($data['teacher_id']) || is_null($data['teacher_id']) || $data['teacher_id'] === '' || trim($data['teacher_id']) === '') {
        if ($existingRecord) {
            // planned_skills kaydını güncelle (teacher_id'yi null yap ve success_status'u güncelle)
            $updateStmt = $pdo->prepare("
                UPDATE planned_skills 
                SET teacher_id = NULL, event_id = NULL, status = :status, success_status = :success_status, updated_at = NOW()
                WHERE id = :id
            ");
            $updateStmt->execute([
                ':id' => $existingRecord['id'],
                ':status' => $data['status'] ?? 'planlandi',
                ':success_status' => $data['success_status'] ?? 'pending'
            ]);
            
            // Diğer organizasyonlardaki aynı becerileri senkronize et
            syncSkillStatusAcrossOrganizations($data['person_id'], $data['skill_id'], $data['success_status'] ?? 'pending', $data['status'] ?? 'planlandi', $pdo);
        }

        echo json_encode(['success' => true, 'message' => 'Eğitmen ataması kaldırıldı ve durum güncellendi.']);
        exit;
    }
