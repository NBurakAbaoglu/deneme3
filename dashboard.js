                ...
                for (let j = i + 1; j < tbodyRows.length; j++) {
                    ...
                    if (cell && !cell.querySelector('img')) {
                        ...
                        img.addEventListener('click', () => {
                            ...
                            console.log('🎯 PIE CHART TIKLANDI:', currentSrc);
                            ...
                            } else if (currentSrc.includes('pie (5).png') || currentSrc.includes('pie%20(5).png') || currentSrc.includes('pie%285%29.png')) {
                                // Eğiticinin Eğitimi formu aç
                                console.log('🎯 Pie (5).png tıklandı - Eğiticinin Eğitimi formu açılıyor...');
                                console.log('🔍 Current src:', currentSrc);
                                console.log('🔍 IMG element:', img);
                                try {
                                    openEgiticininEgitimiModal(img);
                                    console.log('✅ Modal açma fonksiyonu çağrıldı');
                                } catch (error) {
                                    console.error('❌ Modal açma hatası:', error);
                                }
                            } else {
                                // Diğer pie chart'lar için temel beceri formu aç
        ...
        if (cells.length > colIndexnew) {
            ...
            if (!cell.querySelector('img')) {
                ...
                button.addEventListener('click', () => {
                    ...
                    } else if (decodedSrc.includes('pie (5).png') || currentSrc.includes('pie%20(5).png') || currentSrc.includes('pie%285%29.png')) {
                        // Eğiticinin Eğitimi formu aç
                        console.log('🎯 Pie (5).png tıklandı - Eğiticinin Eğitimi formu açılıyor...');
                        console.log('🔍 Current src:', currentSrc);
                        console.log('🔍 Decoded src:', decodedSrc);
...
console.log(getComputedStyle(orgNameEl).display);  // Görünür mü?
    ...
    function openTemelBecerilerModal(imgElement) {
     ...
     // Eğiticinin Eğitimi modalını aç
     function openEgiticininEgitimiModal(imgElement) {
         console.log('🔍 openEgiticininEgitimiModal fonksiyonu çağrıldı');
         
         const modal = document.getElementById("egiticininEgitimiModal");
         modal.style.display = "block";
         modal.currentImg = imgElement;
         
         // Loading durumunu göster
         document.getElementById('egiticininEgitimiLoading').style.display = 'block';
         document.getElementById('egiticininEgitimiContent').style.display = 'none';
         document.getElementById('egiticininEgitimiEmpty').style.display = 'none';
         ...
         if (cell) {
             ...
             const row = cell.closest('tr');
             if (row && organizationId) {
                 window.clickedOrganizationGlobal = organizationId;
                 
                 // Kişi ID'sini al - önce satırdan kontrol et
                 const personIdFromRow = row.getAttribute('data-person-id');
                 if (personIdFromRow) {
                     console.log('🔍 Satırdan person ID alındı:', personIdFromRow);
                     window.clickedPersonGlobal = personIdFromRow;
                     loadEgiticininEgitimiForModal(imgElement);
                 } else {
                     // Satırda yoksa API'den al
                     getPersonIdFromRow(row).then(personId => {
                         console.log('🔍 getPersonIdFromRow sonucu:', personId);
                         window.clickedPersonGlobal = personId;
                         
                         // Modal içeriğini yükle
                         loadEgiticininEgitimiForModal(imgElement);
                     }).catch(error => {
                         console.error('❌ getPersonIdFromRow hatası:', error);
                         window.clickedPersonGlobal = 1; // Varsayılan değer
                         loadEgiticininEgitimiForModal(imgElement);
                     });
                 }
             } else {
                 // Organizasyon ID yoksa varsayılan değerler kullan
                 window.clickedOrganizationGlobal = '1';
                 window.clickedPersonGlobal = 'test';
                 loadEgiticininEgitimiForModal(imgElement);
             }
         }
     }

     // Eğiticinin Eğitimi modalını kapat
     function closeEgiticininEgitimiModal() {
         const modal = document.getElementById("egiticininEgitimiModal");
         modal.style.display = "none";
         modal.currentImg = null;
     }

     // Eğiticinin Eğitimi modal içeriğini yükle
     function loadEgiticininEgitimiForModal(imgElement) {
         console.log('🔄 loadEgiticininEgitimiForModal fonksiyonu başladı');
         
         // Kişi ve organizasyon bilgilerini al
         const personName = getClickedPerson();
         const organizationName = getClickedOrganization();
         
         console.log('🔍 Person Name:', personName);
         console.log('🔍 Organization Name:', organizationName);
         
         // Modal içeriğini oluştur
         const tbody = document.getElementById('egiticininEgitimiTableBody');
         tbody.innerHTML = `
             <tr>
                 <td style="padding: 15px; border-bottom: 1px solid #f0f0f0;">
                     <div style="display: flex; align-items: center; gap: 10px;">
                         <i class="fas fa-chalkboard-teacher" style="color: #2980b9; font-size: 16px;"></i>
                         <div style="font-weight: 600; color: #333; font-size: 14px;">Eğiticinin Eğitimi</div>
                     </div>
                 </td>
                 <td style="padding: 15px; text-align: center; border-bottom: 1px solid #f0f0f0;">
                     <button type="button" class="request-button" onclick="sendEgiticininEgitimiRequest()" style="
                     ...
                     " onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(41, 128, 185, 0.4)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 2px 8px rgba(41, 128, 185, 0.3)'">
                         <i class="fas fa-paper-plane" style="margin-right: 5px;"></i>İstek Gönder
                     </button>
                 </td>
             </tr>
         `;
         ...
         document.getElementById('egiticininEgitimiLoading').style.display = 'none';
         document.getElementById('egiticininEgitimiContent').style.display = 'block';
     }

     // Eğiticinin Eğitimi isteği gönder
     function sendEgiticininEgitimiRequest() {
         console.log('🎯 Eğiticinin Eğitimi isteği gönderiliyor...');
         console.log('🔍 clickedPersonGlobal:', window.clickedPersonGlobal);
         console.log('🔍 clickedOrganizationGlobal:', window.clickedOrganizationGlobal);
         
         // Person ID'yi kontrol et
         let personId = window.clickedPersonGlobal;
         if (!personId || personId === 'test' || personId === null) {
             console.warn('⚠️ Person ID bulunamadı, varsayılan değer kullanılıyor');
             personId = 1; // Varsayılan person ID
         }
         
         const data = {
             person_id: personId,
             organization_id: window.clickedOrganizationGlobal || 1,
             etkinlik_id: 1, // Varsayılan etkinlik ID
             ...
             description: 'Eğiticinin eğitimi isteği',
             status: 'planlandi'
         };
         
         console.log('📤 Gönderilen veri:', data);
         
         fetch('api/save_planned_training_trainer.php', {
             method: 'POST',
             headers: {
                 'Content-Type': 'application/json',
             },
         ...
         .then(data => {
             if (data.success) {
                 showSuccessNotification('Eğiticinin eğitimi isteği başarıyla gönderildi!');
                 closeEgiticininEgitimiModal();
             } else {
     ...
     // Kişi ID'sini al
     function getClickedPersonId() {
         // Global değişkenlerden veya mevcut satırdan kişi ID'sini al
         if (window.clickedPersonGlobal) {
             // Eğer person ID'si string olarak saklanıyorsa
             return window.clickedPersonGlobal;
         }
         
         // Alternatif olarak, mevcut satırdan ID'yi al
         const currentRow = document.querySelector('.excel-table tbody tr:hover');
         if (currentRow) {
             const personIdCell = currentRow.querySelector('[data-person-id]');
             if (personIdCell) {
                 return personIdCell.getAttribute('data-person-id');
             }
         }
     ...
     function getClickedOrganizationId() {
         // Global değişkenlerden organizasyon ID'sini al
         if (window.clickedOrganizationGlobal) {
             return window.clickedOrganizationGlobal;
         }
         ...
         const currentRow = document.querySelector('.excel-table tbody tr:hover');
         if (currentRow) {
             const orgIdCell = currentRow.querySelector('[data-organization-id]');
             if (orgIdCell) {
                 return orgIdCell.getAttribute('data-organization-id');
             }
         }
